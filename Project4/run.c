...
void process_instruction(){
	...
	if(CURRENT_STATE.PIPE_STALL[MEM_STAGE]){
	...
	if (CURRENT_STATE.PIPE_STALL[MEM_STAGE] >= 1){
		CURRENT_STATE.PIPE_STALL[MEM_STAGE] -= 1;
		return;
	//}else if (CURRENT_STATE.PIPE_STALL[MEM_STAGE] == 1){
		//CURRENT_STATE.PIPE_STALL[MEM_STAGE] -= 1;
	}else{
	...
	if (CURRENT_STATE.PIPE[MEM_STAGE]){
		if (CURRENT_STATE.EX_MEM.MemWrite){
			...
			if (!write_cache(CURRENT_STATE.EX_MEM_ALU_OUT, CURRENT_STATE.EX_MEM_W_VALUE)){
				CURRENT_STATE.PIPE_STALL[MEM_STAGE] = miss_penalty;
			}
		}
		if (CURRENT_STATE.EX_MEM.MemRead){
			if (!hit_cache(CURRENT_STATE.EX_MEM_ALU_OUT)){
				CURRENT_STATE.PIPE_STALL[MEM_STAGE] = miss_penalty;
			}
			CURRENT_STATE.MEM_WB_MEM_OUT = find_cache(CURRENT_STATE.EX_MEM_ALU_OUT);
		}
